...
test('storage.init.success', (t) => {
  { //测试多对象访问同一个存储，数据同步的一致性
    const namespace = 'storage.init.with.another.storage'
    const data = { user: { name: 'jhone', age: 19 }, class: 'a' }
    const storage = new Storage(namespace)
    // console.log('storage.data', storage.data, storage.storage.getItem(namespace));
  ...
  { //测试session storage
    const namespace = 'storage.init.with.session'
    const data = { user: { name: 'jhone', age: 19 }, class: 'a' }
    const storage = new Storage(namespace, { useSession: true })
    storage.set(data);
    t.truthy(storage)
    t.is(storage.namespace, namespace)
    t.deepEqual(storage.data, data)

      //测试session storage 和 localstorage 数据隔离
    const another = new Storage(namespace)
    t.truthy(another)
    t.is(another.namespace, namespace)
    t.deepEqual(another.data, {})
...
test.beforeEach((t) => {
  ...
  const namespace = 'storage.test.each'
  const storage = new Storage(namespace)
  storage.clear()
  t.deepEqual(storage.data, {})
...
test('storage.empty.data', (t) => {
  const { storage } = t.context;
  t.deepEqual(storage.set({}), {});
  t.deepEqual(storage.data, {});
