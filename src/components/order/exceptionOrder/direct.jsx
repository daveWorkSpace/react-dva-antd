...
import { Row, Col, Input, Button, Form, Radio, Breadcrumb, Alert, Icon, Popover, Modal, Select, message } from 'antd';
// import Search from '../../operations/search/search';
...
class ExceptionDirectOrder extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      orderId: '',
      flag: 0,        // 1:取消订单状态   2:重新分单状态
      visible: false,
      page: 1,        //初始化分页 
      vendorOrderList: OrderManage.vendorOrderList,       //订单列表                                 //异常订单列表
    };
    ...
    this.private = {
      ...
      orderState: OrderListState.exception,   //异常订单状态-50
      cancelFlag: 1,                        //取消订单状态
      redispatchFlag: 2,                    //重新分单状态
      resetFlag: 0,                         //重置状态
      search: {
        startDate: '20160101',
    ...
    // 服务商订单重新分单请求
    this.renderVendorOrderRedispatch = this.renderVendorOrderRedispatch.bind(this);
  }
  ...
  componentWillReceiveProps(nextProps, nextState) {
    const { dispatch, cancelFlag, redispatchFlag, resetFlag } = this.private;
    ...
    // 更新数据
    if (updateState) {
      if (this.state.flag === cancelFlag) {
        // message.success('取消订单成功')
      }
      if (this.state.flag === redispatchFlag) {
  ...
  renderVendorOrderList() {
    const { dispatch, vendorId, cityCode, orderState, limit } = this.private;
    const { sellerId, orgOrderId, contractType, startDate, endDate, } = this.private.search;
  ...
  // 显示操作提示面板
  showOrHideNoticePanel(flag, state, orderId) {
  ...
  renderCancelOrderPanel() {
    const { itemLayout, cancelFlag, redispatchFlag } = this.private;
    ...
    return (
      <Modal
        ...
        onCancel={() => showOrHideNoticePanel(0, false, '')}
      >
  ...
  orderOperateCallback(e) {
    const { cancelFlag, redispatchFlag } = this.private;
    e.preventDefault();
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (flag === redispatchFlag) {
        const params = {}
        this.renderVendorOrderRedispatch(params)
      }
    });
  }

  // 服务商订单重新分单
  renderVendorOrderRedispatch() {
    const { dispatch } = this.private;
    console.log('重新分单请求')
    // dispatch({ type: closeSingleVendorOrder, payload: params })
  }
  ...
  renderSearchComponent = () => {
    const { cityCode, closeOrderList, closeMeta, } = this.state;
...
export default connect(mapStateToProps)(Form.create()(ExceptionDirectOrder));
