...
//引用通用枚举值
import { DISTRIBUTION_ORDER, DISTRIBUTION_ORDER_COLUMNS } from '../core/enumerate';
...
class DistributionOrder extends Component {
  constructor(props) {
    ...
    this.private = {
      ...
      stockInType: '2,4',     //入库类型(2:配送入3:中转入4:揽收配送入5:揽收中转入)2,4
  ...
  renderStockOrdersList() {
    const { dispatch, vendorId, stockInType, limit, orgOrderId,sellerId, orderState, startTime, endTime } = this.private;
  ...
  onSearch(values) {
    ...
    try {
      /**
       * 解析时间格式，解析utc格式时间为字符串格式如：20170323
       */
      if (values.date[0]) {
  ...
  render() {
    ...
    const tableParams = {
      type: DISTRIBUTION_ORDER,
      columns: DISTRIBUTION_ORDER_COLUMNS(operateCallBack),
      stockOrdersList,
      pagination,
      selectOrderByBatch,
      updateState,
...
export default connect(mapStateToProps)(Form.create()(DistributionOrder));
